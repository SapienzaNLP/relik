...
#             #     set(window2.predicted_spans)
#             # )
#             w1_predicted_spans = set(window1.predicted_spans)
#             w2_predicted_spans = set(window2.predicted_spans)

#             merged_span_predictions = {}
#             for pred in itertools.chain(w1_predicted_spans, w2_predicted_spans):
#                 start, end, label = pred
#                 if (start, end) in merged_span_predictions:
#                     # get probability from first and second window
#                     window1_prob = window1.probs_window_labels_chars.get((start, end), [("None", -math.inf)])[0]
#                     window2_prob = window2.probs_window_labels_chars.get((start, end), [("None", -math.inf)])[0]
#                     if window1_prob[1] > window2_prob[1]:
#                         merged_span_predictions[(start, end)] = (start, end, window1_prob[0])
#                     else:
#                         merged_span_predictions[(start, end)] = (start, end, window2_prob[0])
#                 else:
#                     merged_span_predictions[(start, end)] = (start, end, label)

#             merged_span_predictions = sorted(merged_span_predictions.values())
#             # probabilities
#             for span_prediction, predicted_probs in itertools.chain(
#                 (
#                     window1.probs_window_labels_chars.items()
...
#                     merged_span_probabilities[span_prediction] = predicted_probs
#                 # else:
#                 #     # check if the second one is better than then first
#                 #     if (
#                 #         merged_span_probabilities[span_prediction][0][1]
#                 #         < predicted_probs[0][1]
#                 #     ):
#                 #         merged_span_probabilities[span_prediction] = predicted_probs
#                 #         # change also merged_span_predictions
#                 #         merged_span_predictions.remove((span_prediction[0], span_prediction[1], merged_span_probabilities[span_prediction][0][0]))
#                 #         merged_span_predictions.add((span_prediction[0], span_prediction[1], predicted_probs[0][0]))
#                 #     else:
#                 #         merged_span_predictions.remove((span_prediction[0], span_prediction[1], predicted_probs[0][0]))

            
#             # merged_span_predictions = sorted(merged_span_predictions)


#             # merge probs_window_labels_chars
#             merged_probs_window_labels_chars = window1.probs_window_labels_chars
#             for k, v in window2.probs_window_labels_chars.items():
#                 if k not in merged_probs_window_labels_chars:
#                     merged_probs_window_labels_chars[k] = v
#                 else:
#                     # merge the probabilities of the two windows and sort them
#                     merged_probs_window_labels_chars[k] = sorted(
#                         merged_probs_window_labels_chars[k] + v, key=lambda x: x[1], reverse=True
#                     )
...
#             merged_triplet_predictions,
#             merged_triplet_probs,
#             merged_probs_window_labels_chars
#         )
